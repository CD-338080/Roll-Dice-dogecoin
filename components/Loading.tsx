...
export default function Loading({ setIsInitialized, setCurrentView }: LoadingProps) {
  ...
  const dogeGold = "#F2A900";
  const dogeLight = "#FFD166";
  const dogeDark = "#CB8D00";
  const dogeWhite = "#FFF8E1";

  return (
    <div className="bg-gradient-to-b from-[#F2A900] to-[#FFD166] flex justify-center items-center h-screen overflow-hidden">
      <div className="w-full max-w-xl text-white flex flex-col items-center relative">
        ...
        <div className="absolute inset-0 overflow-hidden">
          {[...Array(20)].map((_, i) => (
            <div 
              ...
              style={{
                backgroundColor: i % 3 === 0 ? dogeGold : i % 3 === 1 ? dogeDark : dogeLight,
        ...
        <div className="w-64 h-64 rounded-full p-4 mb-8 relative z-10">
          <div 
            ...
            style={{
              boxShadow: `0 0 30px ${dogeGold}80`,
              animation: "pulse 2s infinite"
            }}
          >
            <div 
              className="absolute inset-0 bg-gradient-to-br from-[#FFD166] to-[#F2A900] opacity-80"
              style={{
        ...
        <h1 className="text-3xl font-bold mb-4 text-center relative z-10">
          <span className="text-white">Spin</span> <span className="text-[#F2A900]">Claim</span> <span className="text-white">Doge</span>
        </h1>

        {/* Progress bar */}
        <div className="w-64 h-2 bg-[#CB8D00]/50 rounded-full mb-6 overflow-hidden relative z-10">
          <div 
            className="h-full bg-gradient-to-r from-[#F2A900] to-[#FFD166] rounded-full"
            style={{ 
        ...
        <div className="flex items-center space-x-4 relative z-10">
          ...
          <div className="w-10 h-10 relative">
            <div className="absolute inset-0 flex items-center justify-center animate-coin-flip">
              <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-[#F2A900] font-bold text-xl border-2 border-[#CB8D00]">
          ...
          <div className="w-10 h-10 relative">
            <div className="absolute inset-0 flex items-center justify-center animate-coin-flip-delay-1">
              <div className="w-10 h-10 rounded-full bg-[#F2A900] flex items-center justify-center text-white font-bold text-xs border-2 border-white">
                <span className="text-xs">DOGE</span>
              </div>
            </div>
          </div>
          {/* DOGE Coin 3 */}
      ...
      <style jsx global>{`
        @keyframes pulse {
          0% {
            box-shadow: 0 0 15px ${dogeGold}40;
          }
          50% {
            box-shadow: 0 0 30px ${dogeGold}80;
          }
          100% {
            box-shadow: 0 0 15px ${dogeGold}40;
          }
